//! Library target for `simple-groups`, present only so `cargo test --doc`
//! has a target to run against.
//!
//! `simple-groups` is a binary example: `src/main.rs` owns every module.
