...
| Select protocol meaning | Application assembly supplies the applier without giving it lifecycle ownership | `fava/src/lib.rs:340-357`; `fava-write/src/revision.rs:25-45`; Cargo/Bazel provider-path checks are empty | VERIFIED |
| Accept and publish | The edit is accepted with one resolved author, applied, committed, routed, signed, and delivered through the ordinary receipt | `fava-publication/src/lib.rs:87-128`; named first-value and author-custody tests passed | VERIFIED |
| Survive source changes | A newer qualified source reapplies the edit while preserving unrelated state and stable receipt identity | `fava-publication/src/run.rs:238-300`; named reapplication and store-CAS tests passed | VERIFIED |
...
| 3 | A first-value edit applies and publishes through the ordinary receipt | VERIFIED | `Publication::accept` semantic branch; `first_value_edit_publishes_through_public_fava` passed; verifier CLI first-value bundle passed |
| 4 | New qualified source state reapplies a live edit, preserving unrelated changes and stable operation/receipt identity | VERIFIED | Source selection and runner CAS wiring; `newer_source_reapplies_once_and_preserves_unrelated_fields` and `memory_generation_swap_is_compare_and_set` passed |
...
| 6 | Neutral edit/applier boundaries are bounded, authorless, addressable, and reject malformed or mismatched values before effects | VERIFIED | `EventEdit::new`, applier output validation, contract/failure/redb tests, and vocabulary gate |
...
| 8 | Preview/live selection and routing share one path; provider failure, panic, cancellation, capacity, and stale work remain scoped | VERIFIED | `prepare_semantic`/`apply_and_route` shared path; publication/failure targets contain explicit behavioral coverage |
...
| `crates/fava-write/src/revision.rs` | Neutral applier contract and generation identity | VERIFIED | Substantive; selected through public `FavaBuilder` |
...
| `crates/fava-publication/src/{lib,revision,run}.rs` | Selection, revision, reapplication, and ordinary lifecycle | VERIFIED | Public accept/preview/recover paths use the same semantic owner |
| `crates/fava-nip02/src/lib.rs` | First protocol capability | VERIFIED | Follow/unfollow/applier only; exact tests and public API compile checks |
...
|---|---|---|---|---|
| Protocol helper | `EventEdit` | `follow`/`unfollow` and bookmark helpers | WIRED | Public helper output enters `WriteIntent::edit_as` |
| `FavaBuilder` | `Publication` | selected `Arc<dyn EditApplier>` values | WIRED | No private default or core kind dispatch |
| `Publication::accept` | `WriteStore` | reservation, shared revision/route, atomic reserved acceptance | WIRED | Provider effects cannot steal unreserved store capacity |
| Event-cache and write-store sources | reapplication runner | exact query, deterministic winner, source-change loop | WIRED | Independent source closure/failure coverage exists |
| Runner completions | current receipt | write/receipt/revision/event/session/attempt/revision CAS | WIRED | Retired completions refuse without current-state mutation |
| redb schema | recovery runner | persisted edit + author + source/current generation | WIRED | Appliers validated before recovered work starts |
| External N+1 crate | public `fava` | public applier selection and write lifecycle | WIRED | No root workspace membership or private publication dependency |
...
| Publication | Qualified source event | Independent event-cache/write-store snapshots | Protocol applier and current local event | FLOWING |
...
| Query merge | Current local revision | Write-store committed snapshot | Public observation | FLOWING |
| Canary evidence | Receipt/events/revision/source/route facts | Public Fava execution | Seven-file bounded evidence bundle | FLOWING |
...
| Source-v2 reapplication | `cargo test -p fava --test semantic_write_publication newer_source_reapplies_once_and_preserves_unrelated_fields -- --exact` | 1/1 | PASS |
...
| Dropped-notification route recovery | `cargo test -p fava --test semantic_write_publication route_revision::delayed_route_after_reapplication_commits_newer_revision -- --exact` | 1/1 | PASS |
...
| `replaceable-edit-reapplication` | 7 files, 5,570 bytes, exact current revision | PASS |
...
| CAP-03 | 01–05, 08, 09 | SATISFIED | No-prior applier/store/publication/CLI tests |
| CAP-04 | 03, 08, 09 | SATISFIED | Qualified-source reapplication and preservation tests |
