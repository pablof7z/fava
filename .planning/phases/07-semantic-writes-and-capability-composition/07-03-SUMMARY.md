...
tags: [rust, semantic-writes, publication, revision, routing, preview, tdd]
requires:
  - phase: 07-semantic-writes-and-capability-composition
    plan: 01
    provides: bounded edit and applier contracts with exact revision identity
  - phase: 07-semantic-writes-and-capability-composition
    plan: 02
    provides: atomic semantic custody, generation compare-and-set, and recovery facts
provides:
  - public Fava semantic applier selection and first-value publication
  - one exact source observation inside each bounded live semantic receipt runner
  - strict newer-or-empty successor revision with exact self exclusion
...
tech-stack:
  added: []
  patterns:
    - exact kind-indexed applier selection capped before custody
    ...
    - one apply-validate-route derivation shared by preview and live custody
key-files:
  created:
...
key-decisions:
  - "EditApplier identifies its exact non-addressable replaceable Kind while supports validates the edit format; no format noun or registry was added."
...
patterns-established:
  - "Exact semantic selection: author plus kind query, current ReceiptId and RevisionId exclusion, then the standard evaluator winner."
  - "Generation transition: checked injected timestamp, applier, public event validation, route derivation, store CAS, then generation-scoped signing."
...
coverage:
  ...
  - id: D2
    ...
    verification:
      - kind: integration
        ref: "crates/fava/tests/semantic_write_publication.rs#applier_selection_bounds_refuse_before_custody"
        status: pass
    human_judgment: false
  - id: D3
    ...
    verification:
      - kind: integration
        ref: "crates/fava/tests/semantic_write_publication.rs#newer_source_reapplies_once_and_preserves_unrelated_fields"
        status: pass
      - kind: integration
        ref: "crates/fava/tests/semantic_write_publication.rs#own_local_revision_does_not_create_a_second_generation"
        status: pass
      - kind: integration
...
**Public Fava now publishes first semantic revisions, tracks exact live source successors under one bounded receipt runner, and previews the same route without custody or effects.**
...
- Extended the approved applier contract with exact `kind()` selection, capped selection at 64, refused duplicates and unsupported formats, and preserved `supports(&edit)` as format validation.
- Routed first-value and source-backed edits through public `Fava`, ordinary write-store custody, signing, routing, publishing, receipt observation, and stable `RevisionId` evidence.
- Kept cache and write-store source watches open across acceptance inside the existing receipt runner, excluding only the exact current receipt and revision before canonical winner collapse.
...
- Shared exact selection, checked timestamp injection, revision, validation, and route derivation between live custody and semantic preview; preview commits no store or receipt fact and opens no live router, signer, publisher, or transport work.
- Re-exported the existing event, coordinate, edit, revision, and write values needed by external public-facade consumers without adding nominal vocabulary.

## RED and Causal Evidence

- **Task 1 RED:** `cargo test -p fava --test semantic_write_publication` failed at the public facade because the required event, coordinate, edit, applier, timestamp, and intent-error values were not re-exported. Commit: `89a64b1`.
- **Task 2 RED:** the eight-test target compiled; `newer_source_reapplies_once_and_preserves_unrelated_fields` and `source_removal_selects_next_or_empty_once` timed out without generation 2, while `semantic_preview_matches_initial_route_with_zero_effects` received the prior typed refusal. Commit: `841d0c9`.
- **Strict-order deliberate break:** changing the successor comparison from `>` to `>=` made `equal_older_unqualified_and_duplicate_sources_are_inert` observe `RevisionId(2)` instead of `RevisionId(1)`.
...
2. **Task 1 GREEN: Publish first semantic revision** — `0f5e33f` (feat)
3. **Task 2 RED: Specify live reapplication and preview** — `841d0c9` (test)
4. **Task 2 GREEN: Reapply live semantic writes** — `f2995aa` (feat)
...
- `crates/fava-write/src/revision.rs` — applier exact-kind selection method.
- `crates/fava-publication/src/revision.rs` — bounded selection, exact sources, shared derivation, strict successor qualification, and preview support.
...
- Appliers are indexed by the already-approved `Kind`; `supports` remains responsible for edit-format ownership. A registry, descriptor, factory, profile, or compatibility layer was unnecessary.
- Source ownership remains in existing `QuerySource` watches, and durable admission remains atomic in `WriteStore`; a preflight read avoids invoking revision when capacity is already exhausted without replacing store authority.
- Source removal retains the prior timestamp floor and applies from `None`; an older retained event cannot become a fallback generation.
...
**1. [Rule 3 - Blocking contract gap] Added exact applier kind selection and publication query edges**
- **Found during:** Task 1 preflight
- **Issue:** The approved applier contract had no exact kind selector, and `fava-publication` lacked its required `fava-query` Cargo/Bazel edges.
...
- **Files modified:** `crates/fava-write/src/revision.rs`, `crates/fava-publication/Cargo.toml`, `crates/fava-publication/BUILD.bazel`, `Cargo.lock`, `apps/canary/Cargo.lock`, `MODULE.bazel.lock`
