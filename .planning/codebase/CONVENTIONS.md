...
**Functions:**
- Use snake_case and name the observable action or fact: `accept_applied`, `preview_write_routes`, `record_outcome`, `recover_open`, and `run_real_relay_smoke` (`crates/fava-write-store/src/lib.rs`, `crates/fava/src/lib.rs`, `crates/fava-publication/src/run.rs`, `apps/canary/src/lib.rs`).
