...
**Success Criteria** (what must be TRUE):
  ...
  2. `fava-session` exists and owns the account set, signer registrations, and attachment generations; signers can be attached, replaced, and removed at runtime, and an availability transition wakes exactly the matching parked writes.
  3. `fava-publication` no longer owns a signer registry, and no public door mutates a publication lifecycle without passing through its owner - `cancel_write` included.
  4. Phase 07.2's runtime signer work is re-planned on top of this phase rather than beneath it, and its branch is reconciled rather than merged as authored.
