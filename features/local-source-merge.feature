Feature: Local event state is one coherent view
  ...
  # nmp:status=built
  # nmp:evidence=rust:nmp::accepted_local_event_is_visible_without_cache_pollution
  # nmp:falsifier=Ignore WriteStore source contributions; the accepted event disappears and this scenario fails.
  @acceptance
  ...
  # nmp:status=built
  # nmp:evidence=rust:nmp::relay_echo_enriches_one_record_without_erasing_receipt
  # nmp:falsifier=Discard relay evidence while merging a cached and local event id; this scenario fails.
  @acceptance
  ...
  # nmp:status=built
  # nmp:evidence=rust:nmp::cancelling_local_replacement_reveals_cached_predecessor
  # nmp:falsifier=Retain the older candidate when a local replacement is newer; this scenario fails.
  @acceptance
  ...
  # nmp:status=built
  # nmp:evidence=rust:nmp::acquisition_only_and_provenance_constraint_stay_distinct
  # nmp:falsifier=Treat from_relays as a result-provenance constraint; the unrelated local event disappears and this scenario fails.
  @acceptance
  ...
  # nmp:status=built
  # nmp:evidence=rust:nmp::acquisition_only_and_provenance_constraint_stay_distinct
  # nmp:falsifier=Ignore OnlyRelays result authority; the unrelated local event appears and this scenario fails.
  @acceptance
  ...
  # nmp:id=QUERY-OPEN-001
  # nmp:status=built
  # nmp:evidence=rust:nmp-observe::second_source_open_failure_closes_the_first_source
  # nmp:evidence=rust:nmp-observe::initial_evaluation_failure_closes_both_sources
  # nmp:falsifier=Return an open error without explicitly closing provisional source observations; this scenario fails.
  ...
  # nmp:id=QUERY-SOURCE-003
  # nmp:status=built
  # nmp:evidence=rust:nmp-observe::post_open_source_closure_is_scoped_evidence
  # nmp:falsifier=Close the whole query when one local source terminates; this scenario fails.
  ...
  # nmp:id=QUERY-DELIVERY-001
  # nmp:status=built
  # nmp:evidence=rust:nmp::slow_consumer_receives_exact_latest_state_with_bounded_delivery
  # nmp:falsifier=Deliver queued intermediate snapshots instead of the coalesced latest state; the first slow-consumer update is incomplete and this scenario fails.
  @acceptance
