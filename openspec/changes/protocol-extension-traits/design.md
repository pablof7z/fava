...
Dependency direction is preserved and every protocol crate's architecture test passes unchanged.
...
- **A blanket `impl<T: EditApplierSink>` makes the method appear on any future sink implementor.** → Acceptable while `FavaBuilder` is the only one; if a second appears and the blanket impl becomes wrong, the impls narrow to concrete types with no change to callers.
