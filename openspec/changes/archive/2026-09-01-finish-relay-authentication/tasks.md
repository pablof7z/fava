...
- [x] 3.4 (SUPERSEDED 2026-09-01 by `relay-connection-state-machine`, section 5. The hold could not work here: `DeliveryDecision` has no "attempt later" and the destination loop re-decides with no wait, and `AuthenticationOutcomes::state` returns nothing for a session nobody separately asked the owner to watch, so the branch was unreachable in any real assembly. Waiting on the connection's own authentication is what makes it expressible)
...
- [x] 5.3 (DEFERRED 2026-09-01 to `relay-connection-state-machine` task 7.6. Signing a public surface that change rewrites would be signing it twice)
