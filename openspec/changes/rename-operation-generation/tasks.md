...
- [ ] 4.1 Run `cargo fmt --check`, strict Clippy for `fava-query`, `fava-runtime`, `fava-observe`, `fava-diagnostics`, `fava-subscriptions`, and `fava`, and `cargo check --workspace --all-targets`; verify all pass with no compatibility aliases or behavior changes.
...
- [ ] 4.3 Perform a final repository search excluding approved historical evidence and this change's planning artifacts; verify every live declaration, import, re-export, field, accessor, test, diagnostic, and current documentation reference uses `WorkEpoch`/`epoch`, while `RelaySessionGeneration` retains its transport-owned `generation` vocabulary.
