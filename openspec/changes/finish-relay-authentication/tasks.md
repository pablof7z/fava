...
- [x] 3.3 (corrected 2026-09-01: the premise was false — `Nip01Publisher` flattened every `OK` refusal to `PublishOutcome::Rejected`, never reporting `AuthenticationRequired`, and no test of the publisher existed anywhere) Make `Nip01Publisher` report `PublishOutcome::AuthenticationRequired`, carrying the relay's verbatim message, when a relay answers an ordinary `OK` with the `auth-required:` prefix NIP-42 defines; verify `crates/fava-publisher-nip01/tests/refusals.rs` proves both the demand and that every other prefix stays an ordinary rejection, and that reverting the prefix check fails only the first
- [ ] 3.4 (REOPENED 2026-09-01: still not true. Correcting 3.3 made the demand reach the owner, but the hold cannot hold. `DeliveryDecision` has no "attempt later" — only `AttemptNow`, `GiveUp`, `Settled` (`crates/fava-delivery/src/lib.rs`) — and `run_destination` re-decides with no wait, since `read_receipt` sleeps only when the store read errors (`crates/fava-publication/src/run.rs:290`). With the default ceiling of one attempt the `Retryable` arm becomes `GiveUp` on the next pass; a higher ceiling burns every attempt in one tick. No test wires an `AuthenticationOutcomes` reporting `AwaitingAnswer` into a publication, so the branch has never run: `grep -rn AwaitingAnswer` across `crates/**/*.rs` matches only `crates/fava-auth/tests/handshake.rs:165`. Closing this needs a design decision the owner gates — a wait on the retry decision, or waking the publication owner on an authentication change) Hold a publication attempt whose session has a deferred demand instead of recording `AuthenticationDenied` at `crates/fava-publication/src/delivery.rs:196`, waking it on the answer through the path a parked write already uses; verify a test asserts the receipt stays open while the demand is outstanding and completes after approval, alongside the existing WRITE-008 parked-write proof
...
- [x] 5.1 (re-run 2026-09-01 after the doc gate and two transport tests were found failing on `main`; all pass now) Run the full configured validation set — `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, `cargo test --workspace --all-targets --locked`, `cargo test --workspace --doc --locked`, both named falsifiers, and the real-relay proof from 3.1 — and verify every one passes
