...
use crate::croissant_simple_groups_source::{
    FavaSourceProvenance, PinnedFavaExecutable, SOURCE_PATHS, clean_fava_source_against, git, text,
};

const SOURCE_IMAGE: &str = "7777777777777777777777777777777777777777777777777777777777777777";
const SOURCE_MANIFEST: &str = "5555555555555555555555555555555555555555555555555555555555555555";
...
fn production_source_gate_refuses_tracked_and_relevant_untracked_changes() {
    ...
    let build_tree = text(git(repository.path(), &["rev-parse", "HEAD^{tree}"]).unwrap()).unwrap();
    ...
    assert!(source_claim(repository.path(), &executable, &revision, &build_tree, true).is_err());
    assert!(
        Command::new("git")
    ...
    fs::write(
        repository.path().join(".cargo/config"),
        "[net]\noffline=true\n",
    )
...
fn opened_executable_retains_original_after_path_replacement_and_deletion() {
    ...
    pinned
        .retain(&retained)
        .expect("retain opened original image");
