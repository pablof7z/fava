...
`vocabulary-structure.json` is the pinned compiler-derived structure attached
to each term's signed content. It records exact public declarations and paths,
re-exports, fields, variants, methods, signatures, and term-classified private
nominal state. It is generated, never hand-maintained.

`approvals.jsonl` is append-only signed history. One kind-9999 event approves
only the single `name` tag and exact canonical markdown in its content,
including that term's structural JSON. Older events remain preserved after a
candidate or its Rust structure changes; only an event whose content exactly
matches the final current payload is authoritative. A parent term's signature
never approves a child candidate.
...
python3 -m unittest tools.tests.test_vocabulary_check tools.tests.test_vocabulary_structure tools.tests.test_vocabulary_approval
...
Refresh compiler-derived structure after an intentional Rust or registry
change with `python3 tools/vocabulary_structure.py update`. Approval startup
recompiles and refuses a stale snapshot. The UI has no bulk action: each term
shows the literal event content immediately above its own signing button.
