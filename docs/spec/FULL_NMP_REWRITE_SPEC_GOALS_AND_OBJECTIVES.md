# Full NMP Rewrite Specification: Goals and Objectives
...
NMP is an embeddable Nostr client engine with a Rust implementation and first-class platform SDKs. It packages reusable Nostr client behavior—live queries, relay work, local event reuse, signing, routing, publication, receipts, and supporting protocol services—behind an API an application calls.

NMP is a library, not an application framework. The application owns:
...
- which NMP providers and capabilities are compiled into its product.

NMP owns the reusable machinery and lifecycle correctness needed to make those requests work across relays, signers, local data, cancellation, failure, and restart.

NMP has two primary long-lived workload concepts:

1. **Live query.** A declarative request for a current event view. NMP keeps that view coherent as local sources, reactive dependencies, routing knowledge, relay results, and event evidence change.
2. **Write intent.** An accepted publication obligation. NMP carries it through event materialization where needed, signing, routing, delivery, and exact per-destination outcomes under one reattachable receipt.
...
- ownership boundaries between NMP and the application;
...
A concrete application build containing one selected implementation for each required provider contract and zero or more selected capability crates.

Provider selection is a build/application-composition decision. Two different applications may use different event caches, write stores, routers, subscription planners, transports, publishers, delivery policies, signers, and protocol capabilities.

NMP does not promise persistence continuity between unrelated provider implementations merely because they implement the same contract.

### Product profile

A documented NMP assembly with declared guarantees. A profile states, at minimum:
...
- delivery policy;
- selected protocol services and event-kind capabilities; and
- supported platform artifacts.

### Event record

The application-facing representation of one logical event plus the evidence NMP currently has about it.
...
The authoritative owner of accepted local publication obligations, semantic writes, current unsigned or signed materializations, receipts, route revisions, delivery facts, cancellation, supersession, and restart recovery.
...
## GOAL-001 — NMP remains a library

NMP MUST NOT own application navigation, UI state, rendering, ranking, moderation decisions, or product-specific workflows.

NMP MAY provide platform-native reactive and lifecycle wrappers, content parsing, testing infrastructure, and optional UI-independent helpers. Those remain library facilities rather than an application framework.

**Acceptance:** an ordinary application can use NMP while retaining its own state architecture and presentation stack.
...
**Acceptance:** a protocol capability such as follow/unfollow or bookmark/unbookmark produces ordinary query or write values and uses the standard query/publication lifecycles.

## GOAL-003 — Provider selection is static application composition

Applications MUST be able to select different provider implementations without forking NMP or modifying unrelated providers.
...
Changing provider implementation in a later application release is an application/provider migration decision. NMP does not require one implementation to open another implementation's private persisted format.

**Acceptance:** two external applications can compile different storage or routing implementations against the same public contracts with zero NMP source changes.
...
A coordinator MAY order actions between owners. It MUST NOT absorb their policy or duplicate their state.
...
- account and access-context isolation; and
...
| **Protocol service/capability** | NIP/service/event-kind meaning | generic engine lifecycle and primitive query/write paths |
...
A live query MUST state its selection, routing mode, source/access context, freshness policy, cache-use policy, and result/acquisition bounds without embedding application callbacks or hidden side effects.
...
Equivalent observations MAY share local evaluation, relay connections, and wire subscriptions. Distinct source authority, access context, freshness, or presentation-relevant evidence MUST NOT be erased merely because the event filters are equal.
...
- replaceable/addressable winner changes;
- valid deletion;
- expiry;
...
When a query projects event, address, or pubkey references from another event, any valid relay hint carried by that reference—and any compatible relay evidence NMP has for the referenced entity—MAY contribute to automatic routing.
...
## QUERY-009 — NMP never claims global completeness

NMP MUST expose the records it currently has and exact facts for relays and requests it actually used.

NMP MUST NOT expose or imply:
...
When a relay session drops and reconnects, NMP MUST re-establish still-active logical demand with fresh session/request identity and without application resubscription.
...
NMP MUST preserve application-supplied `since`, `until`, and limit semantics.
...
The acquisition window—what NMP should obtain and retain—MUST remain separate from the presentation window—what the UI currently renders.
...
Before relay input can affect any cache, query, routing fact, or application result, NMP MUST:
...
## EVENT-002 — Canonical event semantics are deterministic

Within the facts currently known to an assembly, canonical event state MUST apply deterministic Nostr semantics for:

- exact event-id deduplication;
- replaceable and addressable coordinates;
...
NMP MUST NOT advertise a stronger cache guarantee than the selected implementation provides.
...
When NMP ingests or locally publishes a valid kind:5 deletion, it MUST apply it only to targets the author is permitted to delete and retract affected event records from current queries.

NMP does not automatically widen unrelated application queries merely to fetch deletion events. If a deletion is ingested, it is applied.
...
Such facts MUST remain keyed to the exact relay, request shape, access context, and interval they prove. They MUST NOT become a global sync claim or override application-authored windows.
...
NMP does not promise recovery from an irreparably failing disk or build a product-wide degraded-disk state machine. Ordinary crash/restart semantics remain required for providers that advertise persistence.
...
# Part IV — Event construction, semantic writes, and publication

## WRITE-001 — NMP has one event-construction primitive

NMP MUST expose one general event-construction primitive that supports:

- kind and content;
- validated raw tags as an escape hatch;
- references to users/profiles that produce appropriate `p` tags;
- references to events or addresses that derive correct Nostr reference tags;
- relay and pubkey hints based on trusted target evidence where available; and
- unsigned event output suitable for the one signing/publication path.

Capability crates compose this primitive rather than implementing private tag-building paths.

**Acceptance:** reply, reaction, repost, quote, and custom-kind composition use the same primitive and produce consistent target tags.
...
2. a semantic write containing an actor plus an operation that will materialize into an unsigned event; or
...
## WRITE-003 — Authorship is carried by the event or semantic write

For an unsigned or signed event, the author is the event's `pubkey`.

For a semantic write that has not yet materialized into an event, the actor is part of the semantic write. Every materialization MUST produce an event with that actor as `pubkey`.

Current-account convenience APIs resolve the selected account before producing the accepted event or semantic write. A later account switch MUST NOT retarget accepted work.

No parallel author field may contradict the event or semantic write.
...
- the accepted unsigned event, semantic write, or pre-signed event;
...
## WRITE-006 — Semantic operations survive source changes

A capability may accept an intent-level operation before the final event body is known, for example `Follow(Bob)` by Alice.

The write store MUST retain the semantic operation and actor independently from current materialization. The capability-owned materializer applies the operation over the best qualified source state and may rematerialize when a newer qualified source appears. If no prior source event exists, the capability applies the operation to its defined empty state and produces the first event for that coordinate.
...
Before custody, NMP MUST verify its event id and signature. Publication MUST preserve the exact signed event bytes/identity; routing and delivery MUST NOT mutate it.
...
## WRITE-014 — Routers may acquire their own inputs through NMP primitives

A router owns the meaning and acquisition needs intrinsic to its algorithm. It may request ordinary local reads and explicitly-routed queries through NMP-provided services.
...
If NMP cannot determine whether event bytes reached the destination, it MUST record `OutcomeUnknown` for that exact attempt.
...
When a newer accepted event supersedes an older local replaceable/addressable event at the same coordinate, the older active delivery obligation MUST be retired promptly.

If NMP can prove no bytes were handed off, obsolete delivery state may be removed. If handoff may have happened, bounded explanatory evidence remains.
...
When a semantic write rematerializes into a corrected successor generation, the successor's destination set MUST include:
...
An application may cancel an accepted write while NMP can still prove that no event bytes have been handed to transport for any destination whose obligation cancellation would erase.
...
When a relay serves an event already contributed by the write store, NMP MUST merge relay provenance and delivery evidence into the same event record.
...
A semantic operation that can materialize only into an already-expired event likewise cannot become an active publication obligation.
...
## RELAY-001 — NMP contacts only justified relays

NMP MUST open or retain a relay session only while current query, routing acquisition, authentication, publication, explicit application configuration, or selected service work requires it.
...
It MUST NOT merge across differences that would change semantics, including incompatible time windows, relay-side limits, access contexts, physical sessions, or combinations whose local refiltering cannot reproduce the original result/evidence.
...
When fresh NIP-11 information advertises read/write limits that NMP can interpret deterministically, planning and publication MUST either honor them or surface exact source-scoped shortfall.
...
NMP MUST NOT silently truncate, clamp, collide identifiers, or claim omitted work was completed.
...
The application supplies an auth policy/capability for an exact relay access context. NMP answers challenges, supports challenge timing before or after a request, and re-authenticates after reconnect.
...
Relay `OK`, `CLOSED`, and other user-relevant messages MUST be preserved exactly enough for the application to report what the relay said. NMP MUST NOT replace relay text with an invented generic explanation when the exact message is available.

## RELAY-009 — NIP-11 is a service with independent cache semantics

NMP MUST provide a relay-information service capable of acquiring, validating, caching, and returning NIP-11 documents with exact freshness and acquisition status.
...
NMP MUST NOT automatically introduce negentropy or another set-reconciliation protocol during open, restart, or reconnect. Such a protocol may exist only as an explicit application-selected capability with its own observable contract.
...
NMP MUST keep these outcomes scoped to the exact relay/session/request and must not wedge unrelated work or fabricate stronger facts.
...
Write convenience APIs that use the current account resolve it before producing the accepted unsigned event or semantic write. Accepted work does not follow later account changes.
...
An application may decode `npub`, `nprofile`, or other presentation forms at its input boundary. NMP MUST refuse the wrong identity shape rather than silently reinterpret it where a raw pubkey is required.
...
Applications MUST be able to supply local, remote, hardware, extension, or other signer implementations without giving NMP raw private-key bytes when that provider does not require it.
...
## ID-007 — Signing, encryption, and decryption are separate capabilities
...
# Part VII — Event-kind capabilities and composition

## CAP-001 — Event-kind meaning lives outside the core

The universal query, event-state, routing, publication, and runtime owners MUST NOT branch on optional event-kind capability meaning.

Adding capability N+1 requires a capability crate and product assembly change, not edits to the NMP facade or universal owners.

## CAP-002 — Capabilities compose universal primitives
...
- semantic operations and their antagonists;
- validation;
- presentation-neutral parsed values; and
- materialization logic for replaceable/addressable state.

It uses the one event builder, query model, signer path, router chain, publisher, receipt lifecycle, and write store.

## CAP-003 — Semantic operations expose intent, not whole-event replacement

For replaceable or addressable state, a capability SHOULD expose operations such as:
...
The capability owns how that operation applies to its event coordinate. The write store owns durable custody, materialization generations, signing, routing, delivery, and receipts.

## CAP-004 — Raw Nostr remains expressible
...
Typed capabilities are additive ergonomics and safety. They do not make unknown Nostr inexpressible.

## CAP-005 — Event reference tagging is derived from target facts

When constructing a pointer-like relationship to an event or address, NMP's general event builder MUST derive appropriate root/reply markers, author hints, and usable relay hints from the target's own thread position and trusted evidence.

Capability operations such as reply, reaction, repost, quote, and comment use this same behavior. Non-pointer semantics such as list entries or deletion targets are not forced through pointer tagging.

## CAP-006 — NIP-29 group publication is kind-blind

A selected NIP-29 group publication capability adds the group context and exact route required by the group without restricting the carried event to a fixed set of event kinds.

Custom event kinds can use the same group publication path.

## CAP-007 — NIP-25 reaction construction refuses ambiguous content

The NIP-25 reaction capability MUST refuse content forms that would silently acquire a different protocol meaning than the caller intended, including empty reaction content and shortcode-shaped custom emoji when the required custom-emoji tagging is not supplied.

Refusal happens before write acceptance.

## CAP-008 — NIP-09 deletion remains a protocol write
...
## CAP-009 — Content parsing is shared but presentation-neutral
...
## CAP-010 — Initial target capability inventory is explicit

The rewrite program SHOULD explicitly classify each intended capability as required, optional, deferred, or application-owned rather than letting examples imply support.
...
- explicit opt-in reconciliation capability, if retained.
...
- legacy NIP-04 decryption where required by selected capabilities; and
- Blossom/NIP-96-style asset services where selected.

### Independently selectable event capabilities
...
- NIP-25 reactions;
- NIP-29 groups;
- NIP-51 concepts as separate capability crates rather than one list mega-capability;
- NIP-65 relay-list semantics;
- NIP-73 external identifiers;
- NIP-C7 chat; and
- product-specific capabilities supplied by applications.
...
Opening diagnostics mid-burst returns current truth rather than replaying stale intermediates. With no diagnostics observer, NMP SHOULD avoid constructing expensive presentation snapshots.
...
NMP MUST define bounds or explicit backpressure/refusal for:
...
## OPS-005 — Application-facing test infrastructure is a product capability

NMP MUST ship supported test facilities allowing consuming applications and provider authors to exercise:
...
NMP MUST maintain a real inventory of public operations and values for each supported platform and executable cross-SDK behavior tests.
...
An iOS or Android application MUST consume its selected NMP artifact without repository-relative source paths, raw generated bindings, or direct native-library loading.

The artifact MUST document the selected profile, supported ABI/platform set, and public surface. Unselected capabilities and providers MUST not appear as callable API.
...
Performance qualification must use the production path and report tradeoffs of the selected providers rather than treating one backend's numbers as universal NMP behavior.
...
A production profile exposing NMP write intents MUST select a write store that preserves accepted obligations and receipts across ordinary crash/restart.
...
NMP supplies independently selectable router implementations, including standard outbox, hints, app-relay, and fallback-relay policies.

The application chooses which routers exist and their order. NMP does not centrally impose that app-relay and fallback-relay policy must be combined.
...
NMP MAY publish a recommended full-client assembly, but its implementations use the same contracts and conformance suites as external providers.
...
- selected signer and event-kind capabilities.
...
1. **Application framework ownership.** NMP does not own product UI, navigation, ranking, or moderation policy.
...
4. **Automatic negentropy.** Set reconciliation is explicit opt-in capability work, if supplied.
5. **A core catalog of event-kind meaning.** Capabilities remain independent.
6. **Parallel primitive paths.** Capabilities do not invent their own query, signer, publisher, receipt, or route lifecycle.
...
14. **App-owned reconnect or hidden duplicate retry loops.** NMP/provider owners manage their lifecycles.
...
- NMP remains an embeddable library with live queries and write intents as its primary workload model.
- Applications can select provider and capability crates at build time without forking NMP.
- Every replaceable contract has a public conformance kit and no default-provider privilege.
- Live queries open atomically, return an immediate local view, and remain coherent across additions, removals, provenance changes, semantic rematerialization, and routing changes.
...
- Event authorship comes from the event `pubkey` or the actor on a pre-materialization semantic write, with no contradictory parallel authority.
...
- Optional event-kind capabilities compose universal primitives and adding one does not edit the universal core.
...
| Capabilities | EVT-004/005, NEW-J/K/L/M/N/O, protocol-module ownership requirements |
