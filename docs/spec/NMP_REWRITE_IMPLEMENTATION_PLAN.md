# NMP Rewrite Implementation Plan

**Status:** proposed delivery plan for the rewrite
**Behavioral authority:** `FULL_NMP_REWRITE_SPEC_GOALS_AND_OBJECTIVES.md`
**Architectural authority:** `ARCHITECTURE.md`
**Testing authority:** `NMP_TDD_BDD_TESTING_GUIDE.md`
**Companion acceptance application:** `apps/canary/`

## 1. Purpose

This plan turns the NMP rewrite specification and target architecture into a sequence of complete vertical milestones.

The plan is not organized as "first create every trait, then fill in implementations." Each milestone must produce a usable end-to-end behavior through the public `nmp` facade, add the canary scenario that proves it, and leave behind the narrowest contracts justified by at least one real implementation.
...
- add protocol capabilities without editing the universal core;
- consume equivalent behavior through Rust, Swift, and Kotlin profiles; and
- diagnose and test the real work NMP performs without reaching into internal crates.
...
- the public `nmp` facade;
- selected public capability crates;
- public diagnostics;
...
- an application-side reimplementation of NMP query, route, or receipt semantics; or
- a helper layer that conceals an awkward public NMP API.

### 2.3 One milestone, one coherent capability increase
...
`NMP_TDD_BDD_TESTING_GUIDE.md` defines the development loop and evidence placement. In summary:
...
### E. Services and capabilities

- `nmp-nip11`
- `nmp-nip05`
- `nmp-nip65`
- `nmp-nip02`
- second unrelated capability
- later capability crates selected by product scope
...
| M7 | Semantic writes and capability composition | Follow/unfollow rematerializes over newer source state; capability N+1 changes only its crate and assembly. |
...
Create the independent evidence system used by every later milestone before NMP itself can accidentally become the witness for its own claims.

### Deliverables

- `nmp-e2e-canary` Rust workspace/binary scaffold.
...
- The canary itself has no dependency on NMP internal crates.
...
- local-only portion of `nmp-observe`
- first thin `nmp` facade path

### Required behavior

- Canonical identity, replaceable/addressable winner rules, deletion, expiry, and evidence merge are deterministic.
...
- `nmp-wire`
- `nmp-subscriptions`
- `nmp-subscriptions-no-grouping`
- `nmp-transport`
- `nmp-transport-websocket`
- `nmp-ingest`
- relay-facing portion of `nmp-observe`
- explicit query routing through `nmp`
...
- Open one public NMP query explicitly against that relay.
...
- NMP's public diagnostics and the independent proxy agree on relay/session/subscription identity.
- No NMP internal types appear in the canary.
...
Prove that NMP's live query model remains correct as several relays, local sources, source evidence, reconnect generations, and slow consumers interact.

### Crates/slices

- completion of `nmp-observe`
...
`nmp-router-hints` and `nmp-router-outbox` may begin here or in M6; the primitive must already support them without revision.
...
- `nmp-routing` contains no NIP-65, hint, app-relay, or fallback meaning.
...
- `nmp-write`
- durable `nmp-write-store` implementation, initially Redb or SQLite
- `nmp-signer`
- `nmp-signer-local`
- `nmp-publisher`
- `nmp-publisher-nip01`
- `nmp-delivery`
- initial `nmp-delivery-standard`
- `nmp-publication`
- publication portion of coordinator/runtime
- receipt and stalled-write diagnostics

### Required behavior

- Accepted input forms are unsigned event, semantic write placeholder where supported later, and verified pre-signed event.
...
- `nmp-publication` owns the write lifecycle but not router, signer, publisher, transport, or delivery policy state.
...
- `nmp-router-outbox`
- `nmp-router-hints`
- production `nmp-router-app-relays`
- production `nmp-router-fallback-relays`
...
Prove that event-kind capabilities own meaning while reusing one write lifecycle, and that accepted semantic operations survive changing source state.

### Crates/slices

- `nmp-capability`
- `nmp-nip02`
- a second unrelated capability, recommended: bookmarks or NIP-29 operation
- semantic-operation storage in write store
- materialization/rematerialization lifecycle in publication
- capability conformance kit

### Required behavior

- Capability operations expose intent and antagonists, such as follow/unfollow.
- Capability code produces semantic operations or ordinary event values; it does not sign, route, publish, or own receipts.
- Actor identity exists on the semantic write before materialization; the resulting unsigned event carries it in `pubkey`.
...
- A second capability proves the contract is not secretly NIP-02-shaped.
- Adding capability N+1 edits only its crate and selected assembly/artifact metadata.

### Canary scenarios

`semantic-follow-first-value`

- Alice has no kind-3 event.
- `follow(Bob)` accepts one semantic operation.
- Matching queries immediately show the local materialized kind-3.
- Publication uses the ordinary write receipt.

`semantic-follow-rematerialization`
...
`capability-n-plus-one`

- Add the second capability crate to canary assembly.
- Core crates and `nmp` facade require zero semantic edits.

### Exit gates

- `nmp` has no switch over NIP-02 or the second capability.
- Capability crate dependency direction excludes runtime, transport, store implementation, and standard routers.
- The canary depends on each capability explicitly, making the selected product visible in Cargo metadata.

### Falsifier

Have the capability call a signer/publisher directly. A dependency-negative compile test must fail.
...
- NIP-42 access context is explicit and separate from event authorship/filter identity.
...
- Proxy confirms full EVENT frame crossed from NMP.
- Connection is cut before relay OK reaches NMP.
...
- persistent `nmp-event-cache` implementation
...
- Build a router outside the main NMP workspace.
...
- A capability N+1 and router N+1 change zero universal core lines.
...
- profile/capability selection tooling
- SDK parity inventory
- shared cross-language scenario corpus

### Required behavior

- Native artifacts expose only selected providers/capabilities.
...
The canary is one small real Rust application whose job is to keep NMP honest as a product.

It answers:

> Can an ordinary downstream application read, compose, publish, route, recover, reconnect, inspect evidence, and shut down correctly through the supported NMP facade while NMP performs real work against real relays?

It is not a generic framework or internal debugger.

## 8.2 Application shape

The canary should be a small social client with enough ordinary product behavior to cross NMP's boundaries:
...
- follows feed;
- note/reply/reaction;
- bookmarks or a second semantic capability;
- one NIP-29 room/group flow;
- route preview;
...
- application-visible NMP result;
- public NMP diagnostics;
- independent wire transcript;
- relay process log;
- child-process exit/restart fact; or
- external process resource sample.

NMP diagnostics alone do not prove that a frame crossed the socket. The proxy alone does not prove what the application observed. Important claims compare both.
...
| `hint-routing` | M6 | ROUTER-002, CAP-005 |
| `route-preview-parity` | M6 | WRITE-016 |
| `semantic-follow-first-value` | M7 | CAP-002/003, WRITE-002/006 |
| `semantic-follow-rematerialization` | M7 | WRITE-006/007, CAP-003 |
| `capability-n-plus-one` | M7 | GOAL-006, CAP-001/002 |
