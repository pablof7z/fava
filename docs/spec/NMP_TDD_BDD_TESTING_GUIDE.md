# NMP Testing Strategy: TDD, BDD, and Evidence

**Status:** proposed testing authority for the NMP rewrite
**Behavioral authority:** `FULL_NMP_REWRITE_SPEC_GOALS_AND_OBJECTIVES.md`
**Architectural authority:** `ARCHITECTURE.md`
**Delivery plan:** `NMP_REWRITE_IMPLEMENTATION_PLAN.md`

## 1. Purpose

This document defines how NMP behavior is specified, implemented, and proved.

NMP uses both TDD and BDD, but they serve different purposes:
...
|---|---|
| What NMP must do | `FULL_NMP_REWRITE_SPEC_GOALS_AND_OBJECTIVES.md` |
...
- protocol capability behavior that an app calls directly; and
...
# status: built
# evidence:
#   - crates/nmp-routing/tests/partial_progress.rs
#   - crates/nmp/tests/automatic_route_expansion.rs
...
### `nmp-transport` / `nmp-wire` / `nmp-ingest`
...
### `nmp-publication`, `nmp-publisher`, and `nmp-delivery`
...
Use pure tests for typed decoding and materialization. Add one public publication/query example only when it proves that capability composition reaches the ordinary NMP primitives correctly.
...
Diagnostics report what NMP believes. They do not prove their own claims.
...
- a canary helper that conceals an awkward or missing public NMP API; and
