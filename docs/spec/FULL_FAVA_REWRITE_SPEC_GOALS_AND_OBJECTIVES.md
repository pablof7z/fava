...
Every public query constructor that consumes an externally supplied collection
of authors, ids, kinds, tag values, or explicit relays MUST stop at a declared
`fava-query` resource cap and return an axis-specific `QueryError`. The current
4,096 value is a provisional implementation shortcut, not a Nostr limit or
query-domain semantic. Duplicate or infinite input MUST NOT evade the
consumption cap. There MUST NOT be parallel infallible and fallible iterator
constructors for the same selection axis.
...
For NIP-02, `contact_list(authors)` and `followers_of(subject)` return ordinary
kind-3 `Query` values or the neutral query owner's exact `QueryError`, while
`follows_of(snapshot)` is a typed snapshot projection.
`ContactList` accounts for every `p` tag entry in source order. Valid entries expose
typed pubkey, relay-hint, and UTF-8 petname fields; malformed, duplicate, or
uninterpreted `p` entries expose exact typed entry evidence. Empty lists are valid.
Edits preserve first-occurrence order,
malformed entries, unknown tags, and extensions such as `t` tags while changing
