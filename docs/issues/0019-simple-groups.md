...
`docs/issues/0026-simple-group-relay-input-boundary.md`.

The current nominal model is recorded in `docs/internals/vocabulary.toml`.
The signed approval ledger remains unchanged; this issue does not claim that
governance gate.
...
- `events` must preserve an ordinary query, constrain lowercase `h` to exactly
  ...
  disjoint axes remain present-empty match-nothing queries. Issue 0027 records
...
tests and rustdoc examples required by Pablo's decision recorded in issue 0026.
