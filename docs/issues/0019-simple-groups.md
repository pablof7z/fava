...
`Group` is a pure value containing one opaque NIP-29 group id and an
application-selected, non-empty, bounded host-relay set. One host is the
...
- `GroupSnapshot` deduplicates content by event id while retaining every actual
  `RelayEvidence` observation.
...
- `GroupSnapshot::at` applies an explicit application host choice; no helper
  lets one relay speak for another.

## Query and publication composition

Every helper is inert. Reads return ordinary `Query` values. Unsigned and
signed events pass through pure `Group::prepare`. Saved-list changes return
ordinary `ReplaceableEventEdit` values.

Applications compose publication only through the facade:

```rust
let prepared = group.prepare(payload)?;
let write: Write = fava.to(group.hosts())?.publish(prepared)?;
...
set. Publication remains kind-blind. Invalid signed group context is refused by
pure preparation before Fava custody or wire interaction. Kinds 9002 and 9010
remain complete author-bearing events and never enter `ReplaceableEventEdit`.

## Typed records, discovery, and edits

- `GroupMetadata`, `GroupAdmins`, `GroupMembers`, `GroupRoles`,
  `GroupParticipants`, and `GroupPins` decode signed relay-authored records.
- `PinnedItem`, `SavedGroup`, and `SavedRelay` retain source order and exact
  protocol values without presentation or routing policy.
- `SimpleGroups` creates bounded ordinary saved/admin/member discovery queries
  and projects saving authors from exact group-id and selected-host pairs.
- Kind-10009 saved-group and saved-relay changes preserve opaque content,
  foreign and malformed rows, unrelated order, and exact other-host rows.
...
- `Group`, `GroupError`, `GroupRecords`, `GroupSnapshot`, and `SimpleGroups`.
- `GroupMetadata`, `GroupAdmins`, `GroupMembers`, `GroupRoles`,
  `GroupParticipants`, and `GroupPins`.
- `PinnedItem`, `SavedGroup`, and `SavedRelay`.
...
crates. No group-specific row wrapper, relay role, provider, configuration,
runtime, observation, publication, delivery, cancellation, or receipt value is
approved.
...
4. Universal Fava owners have no production capability dependency, NIP-29
   constant branch, or group-id semantic branch. Generic kind and tag handling
   remains protocol-neutral.
...
state, protocol-specific universal branch, wrong management edit path, group
lifecycle value, or unregistered public export appears.
