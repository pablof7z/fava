...
FEATURE = ROOT / "features" / "nip02-contact-lists.feature"
TEST_SOURCE = ROOT / "crates" / "fava-nip02" / "src" / "tests" / "contact_list.rs"
...
class Nip02ContactListFeatureTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_every_mapping_resolves_to_the_real_cargo_target_and_test(self):
        ...
        for scenario in self.scenarios:
            ...
            with self.subTest(scenario=scenario["name"]):
                self.assertEqual(mapping["package"], "fava-nip02")
                self.assertIn(mapping["target"], targets)
                target = targets[mapping["target"]]
                self.assertTrue(target.get("test") or "test" in target["kind"])
    ...
    def test_parser_refuses_malformed_or_ambiguous_mappings(self):
        malformed = FEATURE.read_text(encoding="utf-8").replace(
            "fava-nip02/fava_nip02#valid_empty",
            "fava_nip02/unit-tests#ValidEmpty",
            1,
        )
        ...
        duplicate = (
            "  # fava:rust=fava-nip02/fava_nip02#one\n"
            "  # fava:rust=fava-nip02/fava_nip02#two\n"
            "  Scenario: ambiguous\n"
        )
