...
fn session(raw: &str, access: RelayAccess) -> RelaySessionKey {
    RelaySessionKey {
        relay: RelayUrl::parse(raw).expect("relay URL"),
        access,
    }
}
...
fn observed(event: Event, session: &RelaySessionKey, at: u64) -> RelayEvent {
    RelayEvent::new(event, session.clone(), Timestamp::from(at))
}
...
#[test]
fn occurrences_are_event_bound_access_exact_and_keep_earliest_time() {
    let author = Keys::generate();
    let alice = Keys::generate();
    let shared = event(&author, Kind::TextNote, 1, "shared", Vec::new());
    let other = event(&author, Kind::TextNote, 2, "other", Vec::new());
    let public = session("wss://relay.example", RelayAccess::Public);
    let authenticated = session(
        "wss://relay.example",
        RelayAccess::Authenticated(alice.public_key()),
    );
    let contributions = vec![
        observed(shared.clone(), &public, 9),
        observed(shared.clone(), &public, 3),
        observed(shared.clone(), &authenticated, 5),
    ];
    ...
    assert!(
        relay_occurrences_for_event(
            shared.id,
            &[contributions[0].clone(), observed(other, &public, 1)],
        )
        .is_none()
    );
}
...
fn immutable_replay_preserves_the_earliest_live_occurrence() {
    ...
    let earliest = observed(immutable.clone(), &public, 3);
    let later_replay = observed(immutable.clone(), &public, 9);
    assert!(
        mutations_for_event(
    ...
    let earlier_replay = observed(immutable, &public, 1);
    assert_eq!(
        mutations_for_event(
            std::slice::from_ref(&earliest),
...
fn replacement_is_partitioned_by_exact_session_and_names_successor() {
    ...
    let authenticated = session(
        "wss://relay.example",
        RelayAccess::Authenticated(Keys::generate().public_key()),
    );
    let old = event(&keys, Kind::Metadata, 1, "old", Vec::new());
    let new = event(&keys, Kind::Metadata, 2, "new", Vec::new());
    let old_public = observed(old.clone(), &public, 1);
    let old_authenticated = observed(old.clone(), &authenticated, 1);
    let incoming = observed(new.clone(), &public, 2);
    assert_eq!(
        mutations_for_event(
            &[old_public, old_authenticated],
            incoming.clone(),
            Timestamp::from(3),
        ),
        vec![
...
fn deletion_and_expiration_retract_every_exact_contribution() {
    let keys = Keys::generate();
    let a = session("wss://a.example", RelayAccess::Public);
    let b = session("wss://b.example", RelayAccess::Public);
