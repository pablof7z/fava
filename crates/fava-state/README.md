...
An admitted `RelayEvent` contains one signed event and one exact relay URL
occurrence. `relay_occurrences_for_event` creates the private, event-id-bound
aggregate used by query records. Replacement is per exact relay; deletion and
expiration may retract several exact contributions.
