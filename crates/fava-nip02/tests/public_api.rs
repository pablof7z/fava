...
use fava_nip02::{
    ContactList, ContactListEntryError, ContactListError, Follow, IntoContactAuthors,
};
...
fn inspect_entry(entry: &ContactListEntryError) -> (usize, &[String]) {
    (entry.source_index(), entry.raw_tag())
}
...
fn external_surface_uses_only_approved_functions_and_types() {
    ...
    let event = EventBuilder::new(author, Kind::ContactList)
        ...
        .tag(Tag::parse(["p", &author.to_hex()]).expect("valid follow entry"))
        .tag(Tag::parse(["p"]).expect("short evidence entry"))
        .build()
    ...
    let evidence: &[ContactListEntryError] = list.entry_errors();
    assert_eq!(inspect_follow(&follows[0]), (0, author, None, None));
    assert_eq!(inspect_entry(&evidence[0]), (1, &["p".to_owned()][..]));
