...
> {
    ...
    for entry in table.iter().map_err(refused)? {
        ...
        // A destination used to carry its own access alongside the receipt's,
        // so the two could disagree if the row was tampered with. A destination
        // is now a bare relay: the receipt's authority is the only record of
        // what it was accepted under, so this pair can no longer disagree —
        // the shape enforces what this check used to.
