    ...
    match message {
        RelayMessage::Event {
            ...
            for filter in &entry.filters {
                let accepted = BTreeMap::from([(id.clone(), filter.clone())]);
                match fava_ingest::admit_subscription_event(
                    ...
                    Ok(_) => return Accepted::Event,
                    Err(error) => refusal = Some(error),
                }
            }
            refusal.map_or(Accepted::Nothing, |error| {
                Accepted::Unattributed(BoundedText::new(error.to_string()))
            })
        }
