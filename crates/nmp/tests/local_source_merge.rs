...
use nmp::{EventQuery, Nmp};
use nmp_event_cache::EventCache;
use nmp_event_cache_memory::MemoryEventCache;
use nmp_query_standard::StandardQueryEvaluator;
use nmp_state::{
    AccessContext, CacheMutation, CachedEvent, RelayEvidence, RelaySessionKey, RelayUrl, Timestamp,
};
use nmp_write::EventValue;
use nmp_write_store::WriteStore;
use nmp_write_store_memory::MemoryWriteStore;
use nostr::event::{
...
fn assembly() -> (Nmp, Arc<MemoryEventCache>, Arc<MemoryWriteStore>) {
...
async fn next_snapshot(feed: &mut nmp_observe::Observation) -> Arc<nmp::QuerySnapshot> {
...
async fn cancelling_local_replacement_reveals_cached_predecessor() {
    ...
    let query = EventQuery::events().kind(Kind::ContactList).cache_only();
    let mut feed = nmp.observe(query).await.expect("query opens");
...
async fn acquisition_only_and_provenance_constraint_stay_distinct() {
    let (nmp, cache, _writes) = assembly();
    let keys = Keys::generate();
    ...
    let from = EventQuery::events()
        .from_relays([asked.clone()])
        .expect("non-empty relay set")
        .cache_only();
    let only = EventQuery::events()
    ...
    let mut only_feed = nmp.observe(only).await.expect("authority query opens");
...
async fn slow_consumer_receives_exact_latest_state_with_bounded_delivery() {
    let (nmp, _cache, writes) = assembly();
    let keys = Keys::generate();
    let mut feed = nmp
        .observe(EventQuery::events().cache_only())
        .await
