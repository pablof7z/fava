...
use fava_runtime::{OperationName, ProviderCompletion, Round, Runtime, RuntimeError, TaskName};

use support::{config, runtime};

const SIGN: OperationName = OperationName("sign_event");

fn generation(value: u64) -> Round {
    static GENERATIONS: OnceLock<Mutex<(RoundIssuer, Vec<Round>)>> = OnceLock::new();
    let index = usize::try_from(value).expect("test generation fits usize");
    let mut state = GENERATIONS
        .get_or_init(|| {
            Mutex::new((
                RoundIssuer::new().expect("test generation authority"),
                Vec::new(),
            ))
        })
        .lock()
...
async fn owner_issued_generations_are_monotonic_and_authority_scoped() {
    ...
    let mut independent = RoundIssuer::new().expect("independent authority");
    assert_ne!(
        generation(1),
