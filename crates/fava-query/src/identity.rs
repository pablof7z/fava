//! Cross-crate read-side identity primitives.
//!
//! These four nominals are named by `fava-subscriptions`' contract
//! (`ARCH:1492-1497`) but are semantically owned by `fava-observe`, which is a
//! lifecycle owner a neutral contract crate must not depend on
//! (`ARCH:3050-3082`). `fava-query` is the lowest crate every dependent
//! already uses, so it defines them and the contract crates re-export them.
