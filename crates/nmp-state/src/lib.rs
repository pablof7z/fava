//! Deterministic semantics for signed event state learned from relays.
...
/// The application-selected authorization context for one relay session.
#[derive(Clone, Debug, Default, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct AccessContext(String);

impl AccessContext {
    /// The ordinary unauthenticated public context.
    #[must_use]
    pub fn public() -> Self {
        Self::default()
    }

    /// Construct a named context without exposing provider-private state.
    #[must_use]
    pub fn named(name: impl Into<String>) -> Self {
        Self(name.into())
    }

    /// Return the stable context name.
    #[must_use]
...
pub struct RelaySessionKey {
    ...
    pub relay: RelayUrl,
    /// Access context under which the event was served.
...
impl RelayEvidence {
    ...
    /// Whether the event was served by a qualifying relay in any access context.
    #[must_use]
...
/// One atomic mutation decided by canonical event-state semantics.
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Canonical identity used for ordinary, replaceable, and addressable events.
#[derive(Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub enum EventCoordinate {
    /// One immutable ordinary event.
    Event(EventId),
    /// Latest event for one author and replaceable kind.
    Replaceable {
        /// Event author.
        author: PublicKey,
    ...
    /// Latest event for one author, addressable kind, and `d` identifier.
    Addressable {
        /// Event author.
        author: PublicKey,
...
/// Determine the canonical coordinate of event-shaped values.
#[must_use]
...
/// Determine the canonical coordinate of a signed event.
#[must_use]
